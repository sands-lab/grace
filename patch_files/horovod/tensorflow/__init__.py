...
def allreduce(tensor, average=None, device_dense='', device_sparse='',
    ...
    if isinstance(tensor, tf.IndexedSlices):
        ...
        with tf.device(device_sparse):
            # print("==Debug== this model has a sparse gradient")
        ...
        with tf.device(device_dense):
            ...
            if grace and op != Adasum:
                # print("==Debug== grace is called")
                new_tensor = grace.step(tensor, name=name)
            else:
                name = None
...
def grouped_allreduce(tensors, average=None, device_dense='', device_sparse='',
                      grace=None, compression=Compression.none, op=None,
                      prescale_factor=1.0, postscale_factor=1.0):
    if not tensors:
        return tensors
...
def _make_allreduce_grads_fn(name, device_dense, device_sparse,
                             grace, compression, sparse_as_dense, op, gradient_predivide_factor,
                             num_groups):
    if op == Average:
...
if _LegacyOptimizer is not None:
    class _DistributedOptimizer(_LegacyOptimizer):
        ...
        def __init__(self, optimizer, name=None, use_locking=False, device_dense='',
                    device_sparse='', grace=None, compression=Compression.none,
            ...
            self._allreduce_grads = _make_allreduce_grads_fn(
                name, device_dense, device_sparse, grace, compression, sparse_as_dense, op,
    ...
    class _DistributedAdasumOptimizer(_LegacyOptimizer):
        ...
        def __init__(self, optimizer, name=None, use_locking=False, device_dense='',
                    device_sparse='', grace=None, compression=Compression.none, backward_passes_per_step=1):
            if name is None:
    ...
    if isinstance(optimizer, _LegacyOptimizer):
        if op == Adasum:
            return _DistributedAdasumOptimizer(optimizer, name, use_locking, device_dense,
                                            device_sparse, grace, compression, backward_passes_per_step)
...
if hasattr(tf, 'GradientTape'):
    class _DistributedGradientTape(tf.GradientTape):
        def __init__(self, tape, device_dense, device_sparse, grace, compression, sparse_as_dense, op,
            ...
            self._tape = tape
            self._allreduce_grads = _make_allreduce_grads_fn(
                'DistributedGradientTape', device_dense, device_sparse, grace, compression,
    ...
    def DistributedGradientTape(gradtape, device_dense='', device_sparse='',
                                grace=None, compression=Compression.none, sparse_as_dense=False,
                                op=Average, gradient_predivide_factor=1.0,
                                num_groups=0):
        ...
        if hasattr(gradtape, '_watch_accessed_variables'):
            return cls(gradtape._tape, device_dense, device_sparse, grace, compression,
                       sparse_as_dense, op, gradient_predivide_factor, num_groups,
