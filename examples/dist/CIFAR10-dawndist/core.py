...
def run_batches(model, batches, training, grc=None, optimizer_step=None, stats=None):
...
def train_epoch(model, train_batches, test_batches, optimizer_step, timer, grc, test_time_in_total=True):
    train_stats, train_time = run_batches(model, train_batches, True, grc, optimizer_step), timer()
...
def train(model, optimizer, train_batches, test_batches, epochs, master_address, rank, grc,
    ...
    for epoch in range(epochs):
        epoch_stats = train_epoch(model, train_batches, test_batches, optimizer.step, timer, grc,
                                  test_time_in_total=test_time_in_total)

        summary = union({'epoch': epoch + 1, 'lr': optimizer.param_values()['lr'] * train_batches.batch_size},
                        epoch_stats)
