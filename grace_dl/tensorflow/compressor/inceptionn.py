...
class INCEPTIONNCompressor(Compressor):
    ...
    def compress(self, tensor):
        ...
        error_bound = 127 + int(math.log(self.error_bound / 2, 10))  # error_bound exponent: 117 for 2e-10
        ...
        def encode_byte(a):
            ...
            a = tf.reshape(a, [-1])
            pad_size = 4 - tf.math.mod(tf.size(a), 4)
    ...
    def decompress(self, tensor_compressed, ctx):
        ...
        indices = tf.expand_dims(indices_32bit, 1)
        temp32 = tf.scatter_nd(indices, v_32bit, [tensor_size])
        indices = tf.expand_dims(indices_16bit, 1)
        temp16 = tf.tensor_scatter_nd_update(temp32, indices, v_16bit)
        indices = tf.expand_dims(indices_8bit, 1)
        tensor_decompressed = tf.tensor_scatter_nd_update(temp16, indices, v_8bit)
